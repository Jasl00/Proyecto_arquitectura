//Integrantes:
//Jorman Solorzano CI:22.515.311
//Jonathan Jaramillo CI: 20.955.384
...
string espaciar(int tamano, int valor){
    int espacio = 0,i;
    string texto = "";

    espacio = valor - tamano;
...
int main(){
    int capacidadCache, numPalabras, numBloques, desplazamiento, bloque, etiqueta, direccion, conjunto, contadorConjunto=0;
    ...
    switch (tipo){
        ...
        case 2:
            ...
            if(conjunto > numBloques) {
                cout << "La cantidad de vias del conjunto es mayor al numero de bloques. El programa se cerrara.\n";
                return 0;
             }
        break;
        case 3:
    ...
    //Se calcula el numero total de bits para el espacio del desplazamiento mediante el log2(tamano del bloque en bytes)
    desplazamiento=log(4*numPalabras)/log(2);   

    //Se calcula el numero total de bits para el espacio del bloque mediante el log2(numero total de bloques en la cache) 
        ...
        do{
            ...
            if(memCache[bitsBloque][contadorConjunto].getEsValido()==1){
                ...
                if(bitsEtiqueta==memCache[bitsBloque][contadorConjunto].getEtiqueta()){
                    ...
                    bandera = true; //Como la etiqueta fue igual, a la buscada es un acierto le suma 1 al LRU, por que acaba ingresar en el
